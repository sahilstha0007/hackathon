    ...
    useEffect(() => {
        const fetchArt = async () => {
            ...
            if (response?.data?.art) {
                setStoreArt(response.data.art.map((art) => ({
                    ...art,
                    randomNumber: Math.floor(Math.random() * 100) + 20, // generate the random number here
                })));
            }
        };
        fetchArt();
    ...
    return (
        <div className='px-28'>
            ...
            <ArtPopUp
                ...
                close={close}
            >
                <button className='absolute top-[10%] left-[89%] text-4xl text-red-600' onClick={close}><MdClose /></button>
            ...
            <div className="grid grid-cols-auto gap-5 pt-5 gap-y-6 sm:px-0 w-full">
                {storeArt.map((art, index) => (
                    <div key={index} className="relative border border-blue-200 rounded-xl overflow-hidden hover:translate-y-[-10px] transition-all duration-500">
                        ...
                        <img
                            className="bg-blue-50 object-cover h-60 w-[100%]"
                            src={art.cover}
                            alt={art.name}
                        />
                            ...
                            <div>
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center">
                                        <button onClick={() => handleLike(art._id)} className="overflow-hidden">
                                            <BiSolidLike
                        ...
                        <p
                            className="cursor-pointer flex justify-center bg-[#4e3cc4] text-white p-2"
                            onClick={() => navigate(`/art/${art._id}`)}
                        >
                            More Details
                        </p>
